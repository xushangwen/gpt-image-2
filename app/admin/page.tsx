...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text-primary)", fontFamily: "var(--font-cn), sans-serif" }}>
      ...
      {/* Header */}
      <header style={{ padding: "0 24px", height: 52, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid var(--border)", background: "var(--surface)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 14, fontWeight: 600, fontFamily: "var(--font-space)" }}>管理后台</span>
          {pendingOrders.length > 0 && (
            <span style={{ fontSize: 11, padding: "2px 7px", borderRadius: 20, background: "#fb923c22", border: "1px solid rgba(251,146,60,0.4)", color: "#fb923c", fontFamily: "var(--font-space)" }}>
              {pendingOrders.length} 待处理
            </span>
          )}
        </div>
        <a href="/" style={{ fontSize: 13, color: "var(--text-muted)", textDecoration: "none" }}>← 返回首页</a>
      </header>

      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 24px" }}>
        {/* Tabs */}
        <div style={{ display: "flex", gap: 4, marginBottom: 20, padding: 4, borderRadius: 10, background: "var(--surface)", border: "1px solid var(--border)", width: "fit-content" }}>
        ...
        {tab === "orders" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <section>
              ...
              {pendingOrders.length === 0 ? (
                <div style={{ padding: "24px", textAlign: "center", borderRadius: 10, border: "1px solid var(--border)", color: "var(--text-muted)", fontSize: 13 }}>
                  暂无待处理订单
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {pendingOrders.map(order => {
                    ...
                    return (
                      <div key={order.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderRadius: 10, border: "1.5px solid rgba(251,146,60,0.3)", background: "rgba(251,146,60,0.04)" }}>
                        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            ...
                            <span style={{ fontSize: 11, padding: "2px 7px", borderRadius: 20, background: "rgba(251,146,60,0.1)", border: "1px solid rgba(251,146,60,0.3)", color: "#fb923c" }}>
              ...
              <section>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {confirmedOrders.map(order => {
                    const pkg = PACKAGES.find(p => p.id === order.package_id);
                    return (
                      <div key={order.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 10, border: "1px solid var(--border)", opacity: 0.7 }}>
                        ...
                        <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: 20, background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.3)", color: "#4ade80" }}>
                          已到账
                        </span>
                      </div>
                    );
        ...
        {tab === "users" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {users.map(u => (
              <div key={u.user_id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface)" }}>
